...
- [x] Refinement loop: поле уточнений под результатом → POST /adapt/refine → обновление на месте
- [x] Исправлены буллеты: промпт явно требует "-" вместо "*"
- [x] Заголовки секций: ALL CAPS в промпте + построчный рендерер с визуальным выделением
